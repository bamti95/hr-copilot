...
5. 평가가이드는 면접관이 답변을 들으며 바로 사용할 수 있는 진행 가이드처럼 씁니다.

평가가이드 작성 원칙:
- 단순히 "구체적으로 답하면 좋음"처럼 쓰지 마세요.
- 면접관이 어떤 관점으로 답변을 들어야 하는지 방향을 제시하세요.
- 좋은 답변, 보통 답변, 부족한 답변의 차이를 구분해 주세요.
- 필요하면 후속 확인 포인트를 포함하세요.
- 문서에 없는 수치나 성과를 정답처럼 요구하지 말고, 확인해야 할 탐색 포인트로 제시하세요.

평가가이드 권장 형식:
관찰 포인트: 답변에서 반드시 확인할 핵심 역량/경험
좋은 답변 기준: 맥락, 본인 역할, 의사결정 기준, 결과/학습을 구체적으로 설명
보통 답변 기준: 경험은 설명하지만 역할/판단 기준/결과가 일부 부족
부족한 답변 기준: 문서 내용을 반복하거나 본인 기여와 근거가 불명확
후속 확인: 답변이 모호할 때 추가로 확인할 지점
...
evaluation_guide는 면접관용 실전 가이드입니다.
반드시 아래 내용을 한 문단 또는 짧은 항목으로 포함하세요.
- 관찰 포인트
- 좋은 답변 기준
- 보통/부족한 답변 기준
- 답변이 애매할 때 확인할 후속 포인트
""".strip()
...
평가가이드 심사 기준:
- 면접관이 답변을 들으며 무엇을 관찰해야 하는지 알려주는가
- 좋은 답변/보통 답변/부족한 답변의 차이가 분명한가
- 지원자 문서에 없는 수치나 정답을 강요하지 않는가
- 답변이 모호할 때 어떤 후속 확인을 해야 하는지 방향을 주는가
- 단순 키워드 나열이 아니라 실제 면접 진행에 도움이 되는가
...
- evaluation_guide가 막연하면 requested_revision_fields에 evaluation_guide를 포함합니다.
