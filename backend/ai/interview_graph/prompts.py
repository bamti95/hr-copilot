...
Analyzer 결과를 바탕으로 실제 면접에서 사용할 수 있는 질문 후보 10개를 생성합니다.
...
- category는 반드시 다음 한국어 표준 카테고리 중 하나만 사용합니다:
  기술 역량, 직무 역량, 경험 검증, 리스크 검증, 조직 적합성, 지원 동기, 커뮤니케이션, 기타
- 임의 카테고리명, 영어 코드, 축약 표현은 사용하지 않습니다.
...
언어 규칙:
- 모든 답변은 한국어로 작성합니다.
- 면접관이 참고할 수 있는 핵심 예상 답변 요약으로 작성합니다.
- predicted_answer는 2~3문장, 300자 이내로 작성합니다.
- predicted_answer_basis는 1문장, 160자 이내로 작성합니다.
...
- 장황한 상황 설명, 가상의 PR 번호, 구체 코드명, 긴 대화체 답변은 작성하지 마세요.
...
- predicted_answer는 2~3문장, 300자 이내의 간결한 요약으로 작성하세요.
- 답변은 실제 지원자가 말할 법한 핵심만 담고, 상세 사례 서술은 꼬리 질문에서 검증하도록 남겨두세요.
