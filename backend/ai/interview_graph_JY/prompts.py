...
본 질문, 문서 분석, 선택적으로 제공되는 예상 답변에서 가장 검증 가치가 높은 지점을 파고드는 꼬리질문을 만듭니다.

규칙:
- 원 질문을 반복하지 않습니다.
- 예상 답변이 비어 있거나 부족하면 질문의 평가 가이드, 문서 근거, 리스크 태그, 문서 분석을 기준으로 꼬리질문을 생성합니다.
- 예상 답변이 제공된 경우에는 답변의 불명확한 역할, 수치, 의사결정, 리스크를 우선 검증합니다.
