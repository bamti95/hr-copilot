...
### 2.1 `document` — 업로드 문서 원본

> Role Profile / 이력서 / 포트폴리오 파일을 통합 관리. OCR/파서 결과 텍스트 보관.
...
**제약조건**
- `CHECK: document_type IN ('ROLE_PROFILE','RESUME','PORTFOLIO')`
- `CHECK: extract_status IN ('PENDING','READY','FAILED')`
- `FK: created_by → manager.id`
- `FK: deleted_by → manager.id`
...
### 3.2 `prompt_profile` — 프롬프트 실행 전략 프로파일
...
### 3.3 `prompt_profile_template` — 프로파일-템플릿 조립 매핑
...
> 1번의 LLM 분석 실행을 나타내는 헤더. 문서 3종(Role/Resume/Portfolio)과 프로파일을 연결.  
> `assembled_prompt`에 실제 전달된 최종 프롬프트를 저장해 디버깅 및 이력 추적에 활용.
...
document ──1:N──▶ workflow_run              (role / resume / portfolio 3개 FK)
